...
class reseauBayesien():
    ...
    def lire_reseau(self):
        with open(self._file_name,'r') as file :
            ...
            line=file.readline().split()[3::2] # line devient : ['F', 'Fumeur', 'V', 'Video', 'C', 'Capteur', 'A', 'Alarme']
            self._noeuds=(line[::2],line[1::2]) # self._noeuds devient (['F', 'V', 'C', 'A'], ['Fumeur', 'Video', 'Capteur', 'Alarme'])
            file.readline()
            ...
            for line in file :
                ...
                noeud = seq[0].strip()
                parents = []
                if len(seq[1].strip().split())>0 : # si il y a des parents
                    #récupération des parents du noeud
...
# requette=nouveauRB.creer_requette("P(F=1 | C=0, A=1)")
estimation_simple(rb, requette, "F") # retourne P(F|non(C),A)
